...
function containsProhibitedKeywords(text) {
    return PROHIBITED_KEYWORDS.some(keyword => text.toLowerCase().includes(keyword));
}
...
app.post('/send_message', (req, res) => {
    ...
    if (sender_id && message) {
        if (containsProhibitedKeywords(message)) {
            return res.status(400).json({ status: 'Message contains prohibited content' });
        }
        const timestamp = new Date().toISOString();
...
wss.on('connection', (ws, req) => {
    ...
    ws.on('message', (message) => {
        try {
            ...
            if (parsedMessage.type === 'MESSAGE' && containsProhibitedKeywords(parsedMessage.content)) {
                ws.send(JSON.stringify({ type: 'ERROR', message: 'Message contains prohibited content' }));
                return;
            }
...
server.listen(2306, 'localhost', () => {
    console.log('Server started on http://localhost:2306');
});
